...
const EditQuestion = (props) => {
    ...
    const { editquestiondata } = props;
    console.log(editquestiondata)
    const { id, deficulty_level, question_type, question, subject, option_a, option_b, option_c, option_d } = editquestiondata;
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            ...
            axios.post("http://localhost:4000/q/updatequestion", formData).then(res => {
                console.log(res)
                if (res.status === 200) {
                            ...
                            <div className="p-10">
                                <div className="grid grid-cols-12 gap-6">
                                    <div className="col-span-3">
                                        <Select label="Select Question type" color="purple" value={formik.values.qtype} onChange={e => { formik.handleChange(e); setDropvalues('qtype', e) }} name="qtype">
                                    ...
                                    {
                                        (formik.values.file === null || questiontype) &&
                                        <div className="col-span-9" >
                                            <Textarea label="Question" color="purple" value={formik.values.question} name="question" resize={true} onChange={formik.handleChange} />

                                            <span className="text-red-800"> {formik.errors.question ? <div>{formik.errors.question}</div> : null}</span>
                                        </div>
                                    }
                                    {
                                        (formik.values.file != null && questiontype === false) &&
                                        <div className="col-span-9">
                                                    ...
                                                    <div className="mt-4 flex text-sm leading-6 text-gray-600 ">
                                                        <label className="block ">
                                                            <input type="file"
                                                                ...
                                                                className="block w-full text-sm text-slate-500  file:rounded-full file:border-0 "
                                                            />
                                                        </label>
                                                        <span>  <img className="h-24 w-16" src={formik.values.file} alt="" /></span>
                                ...
                                <div className="grid grid-cols-12 gap-6 py-6">
                                    <div className="col-span-3 ">
                                        <Select label="Select Subject" color="purple" value={formik.values.subject} name="subject" onChange={e => { formik.handleChange(e); setDropvalues('subject', e) }} >
                                    ...
                                    <div className="sm:col-span-3">
                                        <Select label="Select Deficulty Level" color="purple" value={formik.values.deficulty} name="deficulty" onChange={e => { formik.handleChange(e); setDropvalues('deficulty', e) }}>
                                ...
                                <div className="grid grid-cols-12 gap-6 py-2">
                                    <div className="col-span-3">
                                        <Input color="purple" label="Option A" value={formik.values.optionA} name="optionA" onChange={formik.handleChange} />
                                        <span className="text-red-800"> {formik.errors.optionA ? <div>{formik.errors.optionA}</div> : null}</span>
                                    </div>

                                    <div className="col-span-3">
                                        <Input color="purple" label="Option B" value={formik.values.optionB} name="optionB" onChange={formik.handleChange} />
                                        <span className="text-red-800"> {formik.errors.optionB ? <div>{formik.errors.optionB}</div> : null}</span>
                                    </div>
                                    <div className="col-span-3">
                                        <Input color="purple" label="Option C" value={formik.values.optionC} name="optionC" onChange={formik.handleChange} />
                                        <span className="text-red-800"> {formik.errors.optionC ? <div>{formik.errors.optionC}</div> : null}</span>
                                    </div>
                                    <div className="col-span-3">
                                        <Input color="purple" label="Option D" value={formik.values.optionD} name="optionD"
                                            onChange={formik.handleChange} className='' />
